...
rank_t handRank(char const hand[5]){
	...
	// find max
	for(int i = 0; i < 13; i++){
		if(i != cardStrength('J') && qty[i] > maxQty){
			maxQty = qty[i];
			max = i; 
		}
	}
	...
	// find second max
	for(int i = 0; i < 13; i++){
		if(i != cardStrength('J') && i != max && qty[i] > max2Qty){
			max2Qty = qty[i];
			max2 = i; 
		}
	}

	// sum joker to biggest
	switch(maxQty + qty[cardStrength('J')]){
		case 5:
...
int main(int argc, char**argv){
	...
	uint64_t win = 0;
	// for(int i = 1000 - 5 ; i < 1000; i++){
	// 	uint64_t a = (i + 1 -(1000 - 5));
	// 	uint64_t b = a * play.hands[i].bid;
	// 	win += b;
