...
// file from stdin (piped)
void *dataFromFile(FILE *file, size_t *filesize){
...
void *dataFromFilename(const char *filename, size_t *filesize){
    FILE *file = fopen(filename, "r+b");
    if(file == NULL) return NULL;

    return dataFromFile(file, filesize);
}
